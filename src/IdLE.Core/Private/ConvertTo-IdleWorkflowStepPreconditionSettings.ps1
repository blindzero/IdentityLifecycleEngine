...
function ConvertTo-IdleWorkflowStepPreconditionSettings {
    ...
    if (Test-IdleWorkflowStepKey -Step $Step -Key 'Precondition') {
        ...
        if ($null -ne $rawPrecondition) {
            ...
            $stepWarnings = [System.Collections.ArrayList]::new()
            Assert-IdleConditionPathsResolvable -Condition ([hashtable]$rawPrecondition) -Context $PlanningContext -StepName $StepName -Source 'Precondition' -AllowMissingRequestContextPaths -WarningSink $stepWarnings

            foreach ($warningItem in @($stepWarnings)) {
                if ($null -eq $warningItem) {
                    continue
                }

                $warningMessage = [string](Get-IdlePropertyValue -Object $warningItem -Name 'Message')
            ...
            $normalized.Warnings = @($stepWarnings | ForEach-Object { Copy-IdleDataObject -Value $_ })
