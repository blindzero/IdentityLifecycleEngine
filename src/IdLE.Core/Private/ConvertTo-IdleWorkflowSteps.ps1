...
function ConvertTo-IdleWorkflowSteps {
    ...
    foreach ($s in @($WorkflowSteps)) {
        ...
        $preconditionSettings = ConvertTo-IdleWorkflowStepPreconditionSettings -Step $s -StepName $stepName -PlanningContext $PlanningContext -WarningSink $planWarnings
        ...
        $preconditionWarnings = @($preconditionSettings.Warnings)
