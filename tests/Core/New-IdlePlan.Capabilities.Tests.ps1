...
Describe 'New-IdlePlan - required provider capabilities' {
    ...
    Context 'Missing capabilities' {
        It 'fails fast when required capabilities are missing' {
            $wfPath = Join-Path -Path $script:FixturesPath -ChildPath 'joiner-missing-caps.psd1'
    ...
    Context 'Metadata validation' {
        It 'rejects host override attempt of step pack metadata (DuplicateStepTypeMetadata)' {
            $wfPath = Join-Path -Path $script:FixturesPath -ChildPath 'joiner-override.psd1'
        ...
        It 'rejects metadata with ScriptBlock values' {
            $wfPath = Join-Path -Path $script:FixturesPath -ChildPath 'joiner-scriptblock.psd1'
        ...
        It 'rejects invalid metadata shapes' {
            $wfPath = Join-Path -Path $script:FixturesPath -ChildPath 'joiner-invalid.psd1'
        ...
        It 'rejects invalid capability identifiers' {
            $wfPath = Join-Path -Path $script:FixturesPath -ChildPath 'joiner-invalid-cap.psd1'
