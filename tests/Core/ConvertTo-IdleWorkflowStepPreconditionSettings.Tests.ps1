...
Describe 'ConvertTo-IdleWorkflowStepPreconditionSettings' {
    InModuleScope 'IdLE.Core' {
        ...
        It 'does not throw when precondition uses unresolved Request.Context path in planning context' {
            ...
            $planningContext = @{ Plan = @{ LifecycleEvent = 'Joiner' }; Request = @{ IdentityKeys = @{}; Intent = @{}; Context = @{} } }
            $warningSink = [System.Collections.ArrayList]::new()

            $result = $null
            { $result = ConvertTo-IdleWorkflowStepPreconditionSettings -Step $step -StepName 'MissingPath' -PlanningContext $planningContext -WarningSink $warningSink } | Should -Not -Throw

            @($warningSink).Count | Should -Be 1
            $warningSink[0].Code | Should -Be 'PreconditionContextPathUnresolvedAtPlan'
